...
from .forms import StudentAchievementForm, StudentAchievementVerificationForm
...
def verify_achievement(request, achievement_id):
    ...
    if request.method == 'POST':
        form = StudentAchievementVerificationForm(request.POST, instance=achievement)
        faculty = Faculty.objects.get(user=request.user)

        if form.is_valid():
            achievement = form.save(commit=False)
            achievement.approved_by = faculty
            achievement.save()

    else:
        form = StudentAchievementVerificationForm(instance=achievement)

    return render(request, 'home/verify_achievement.html', { 'achievement_details': details, 'form': form })
